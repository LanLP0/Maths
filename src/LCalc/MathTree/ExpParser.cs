...
internal sealed class ExpParser
{
    ...
    public ExpParser() : this(new Scope())
    {
    }

    public ExpParser(Scope scope)
    {
        Scope = scope;
    }
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            switch (c)
            {
                ...
                case '.': // Special number / Number
                {
                    ...
                    if (chunk.Length is 1 && c is '0' && i + 2 < chunks.Length)
                    {
                        // Test for special number
                        var nextChunk = chunks[i + 1];
                        if (nextChunk.Length is 1 && nextChunk.GetFirst(math) is 'b' or 'x' or 'o')
                        {
                            var nextNextChunk = chunks[i + 2];
                            if (GetCharacterType(nextNextChunk.GetFirst(math)) is CharacterType.Number)
                            {
                                // This is a special number
                                // Replace this chunk with the one that represent
                                // the whole special number
                                chunk = new SpanSegment<char>(chunk.Start,
                                    chunk.Length + nextChunk.Length + nextNextChunk.Length);
                                ...
                                i += 2; // Skip over the two chunks that have ben merged
                                break;
                            }
                        }
                    }
                ...
                case '&': // Variable / bw and
                {
                    if (math.TryGetValueAt(chunk.Start + 1, out var nextChar) && nextChar.IsLowerLetter())
                    {
                        ...
                        var length = chunk.Length + chunks[i + 1].Length;
                        var skip = 1;
                        if (i + 3 < chunks.Length)
                        {
                            ...
                            if (next2.GetFirst(math) is '=')
                            {
                                ...
                                if (next3FirstChar is '(')
                                {
                                    // TODO compute in variable asignment
    ...
    private Result MoveUpToTop(scoped ref LinkedNode<IMathNode?> levelRoot)
    {
        for (;;)
        {
            ...
            var result = MoveUp(ref levelRoot);
            if (result.Faulted)
                return result;
        }
    }
    ...
    private Result SetVariable(SpanSegment<char> segment, ReadOnlySpan<char> math)
    {
        ...
        if (GetCharacterType(value[0]) is CharacterType.Number)
        {
        ...
        ref Span<SpanSegment<char>> output, out int count)
    {
        ...
        for (var i = 1; i < math.Length; i++)
        {
            ...
            if (type == prevType && type != CharacterType.Other)
                continue;

            output[count] = new SpanSegment<char>(start, i - start + 1);
